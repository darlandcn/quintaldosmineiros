  ...
  <div class="bg-white rounded-2xl border border-[#2F5946]/20 shadow-[0_1px_4px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col h-full min-h-0">
    ...
    <div class="shrink-0 px-6 py-4 border-b border-zinc-100 flex items-center justify-between">
      <div class="space-y-0.5">
        ...
        <p class="text-base font-bold text-gray-900 leading-none">Recentes</p>
      </div>
      <span class="text-[11px] text-zinc-400 font-medium">Últimos 5</span>
    </div>

    <!-- Table wrapper com scroll interno -->
      ...
      <div v-if="loading" class="divide-y divide-zinc-50 animate-pulse">
        <div v-for="i in 5" :key="i" class="flex items-center gap-4 px-6 py-3.5">
          ...
          <div class="flex items-center gap-3 flex-1 min-w-0">
            <div class="w-7 h-7 rounded-full bg-gray-100 shrink-0" />
      ...
      <table v-else class="w-full">
        <thead class="sticky top-0 bg-white">
          <tr class="border-b border-zinc-100">
            ...
            <th class="text-right px-6 py-3 text-xs font-medium text-zinc-500">Status</th>
          </tr>
        </thead>
        <tbody>
          <tr
    ...
    <!-- Footer -->
    <div class="shrink-0 px-6 py-3 border-t border-zinc-100">
